...
public class TransformFunctions : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKet(KeyCode.UpArrow)) transform.Translate(Vector3.forward * moveSpeed * Time.deltaTime);

        if (Input.GetKey(KeyCode.DownArrow)) transform.Translate(-Vector3.forward * moveSpeed * Time.deltaTime);

        if (Input.GetKey(KeyCode.LeftArrow)) tranform.Rotate(Vector3.up, -turnSpeed * Time.deltaTime);
